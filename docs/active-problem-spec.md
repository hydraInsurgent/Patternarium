...
#### Solution
**Time:** O(?) | **Space:** O(?)
**Key Condition:** [The specific formula, invariant, or condition - e.g. `length - maxFreq <= k`. Omit if the approach has no central formula.]
**Key Idea:** [One sentence summarizing the approach]
...
## Connections
- **[Number] - [Problem Name]**: [one-sentence connection - e.g. "same center expansion technique, different bookkeeping goal"]
...
## Import Notes
<!-- Only present when this session was created via /import-chat. Removed after /save-problem. -->
<!-- Content here is synthesized from the analysis file - not copy-pasted. Each entry reflects the session context. -->

### Constructs Identified
<!-- Role of each construct in this specific session's approach. Not a bare name list. -->
- [construct]: [how it was used here, e.g. "Dictionary<char,int> - frequency map reset per starting index i, not per j"]

### Remaining Approaches
<!-- Actionable next steps based on what the user already understands. Include what clicked and what open question remains. -->
- [Approach name]: [e.g. "Sliding Window - you understand the brute force structure; open question: what condition lets you advance the left pointer?"]

### Session Checklist
<!-- Filtered from analysis file. Items already addressed by the approach blocks above are dropped. -->
#### Overall
...
[approach-specific items - only those that remain genuinely open]
```
...
| Connections to other problems are mentioned | Write `## Connections` (optional - only if connections come up naturally) |
| User answers reflection questions | Write `## Reflection` |
| `/import-chat` completes | Append `## Import Notes` with synthesized constructs, remaining approaches, and filtered checklist - only for imported sessions |
...
| All solved approaches from `solutions.md` | `master-index.json` - regenerated by running `dotnet run scripts/Rebuild-Index.cs` |
...
2. Ask: "There is an unfinished session for [Problem Name]. (a) Save it with /save-problem, (b) Pause it with /pause-problem, (c) Discard and start fresh, (d) Resume it"
3. Act on user's choice (discard clears both files to empty)
