...
The learning journey file. Contains YAML frontmatter (machine-readable metadata used by Dataview and the rebuild script) followed by a markdown body (human-readable learning journey).
...
`master-index.json` is a generated build artifact - the cross-reference index for all solved problems. It is never written by AI directly. After every `/save-problem`, it is regenerated by running:

```
dotnet run scripts/Rebuild-Index.cs
```

The script reads `solutions.md` YAML frontmatter from every problem folder and produces:
...
- `by-pattern`, `by-ds`, `by-construct`, `by-algorithm` - reverse-lookup indexes, generated from `solutions.md` frontmatter
...
The single source of truth for this file is `scripts/Rebuild-Index.cs`. Dataview queries read problem frontmatter directly.
...
- On `/save-problem`, update or create the algorithm file; add the `slug` to the `algorithms` array in the problem's `solutions.md` frontmatter (the `## Seen In` section in the algorithm file is a Dataview query and updates automatically); run Step 7 to regenerate master-index.json
