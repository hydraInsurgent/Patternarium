...
- **Array constructor takes length, not last index** - `new bool[n, n]` where `n = s.Length - 1` creates an array missing the last position (indices 0 to `s.Length - 2`). Always pass `s.Length` (element count), not `s.Length - 1` (last index). Same rule applies to 1D and 2D arrays. (Palindromic Substrings, Approach 4)
- **`string.Substring(startIndex, length)` - second argument is length, not end index** - to extract `s[i..j]` use `s.Substring(i, j - i + 1)`. Passing the end index directly causes `ArgumentOutOfRangeException` when the end index is larger than the remaining length. (Palindromic Substrings, Approach 2)
- **`string.Reverse()` returns `IEnumerable<char>`, not a string** - calling `.ToString()` on it gives the type name, not the reversed string. Correct form: `new string(s.Reverse().ToArray())`. (Palindromic Substrings, Approach 2)
