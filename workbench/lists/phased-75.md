# Phased 75

**Progress: 5 / 75** | Easy: 2/19 | Med: 3/49 | Hard: 0/7

## Phase 1 - Foundation (Problems 1-20)

### Batch 1 - Warm-up + Confidence

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 217 | Contains Duplicate | Easy | not started | - | - |
| 242 | Valid Anagram | Easy | not started | - | - |
| 1 | Two Sum | Easy | solved | HashMap - Complement Lookup | [→](../../problems/1-two-sum/problem.md) |
| 20 | Valid Parentheses | Easy | not started | - | - |
| 268 | Missing Number | Easy | solved | Odd One Out | [→](../../problems/268-missing-number/problem.md) |
| 5 | Longest Palindromic Substring | Med | solved | Two Pointers - Expand Around Center | [→](../../problems/5-longest-palindromic-substring/problem.md) |

### Batch 2 - Slight Stretch

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 238 | Product of Array Except Self | Med | not started | - | - |
| 121 | Best Time to Buy and Sell Stock | Easy | not started | - | - |
| 125 | Valid Palindrome | Easy | solved | Two Pointers - Symmetry Check | [→](../../problems/125-valid-palindrome/problem.md) |
| 647 | Palindromic Substrings | Med | not started | - | - |
| 3 | Longest Substring Without Repeating Characters | Med | solved | Sliding Window | [→](../../problems/3-longest-substring-without-repeating-characters/problem.md) |
| 424 | Longest Repeating Character Replacement | Med | not started | - | - |
| 76 | Minimum Window Substring | Hard | not started | - | - |

### Batch 3 - Thinking Shifts

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 11 | Container With Most Water | Med | not started | - | - |
| 206 | Reverse Linked List | Easy | not started | - | - |
| 21 | Merge Two Sorted Lists | Easy | not started | - | - |
| 141 | Linked List Cycle | Easy | not started | - | - |
| 23 | Merge k Sorted Lists | Hard | not started | - | - |

### Batch 4 - First Real Challenge

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 15 | 3Sum | Med | not started | - | - |
| 53 | Maximum Subarray | Med | not started | - | - |
| 153 | Find Minimum in Rotated Sorted Array | Med | not started | - | - |
| 33 | Search in Rotated Sorted Array | Med | not started | - | - |

### Batch 5 - Pattern Reinforcement

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 49 | Group Anagrams | Med | not started | - | - |
| 128 | Longest Consecutive Sequence | Med | solved | Linear Scan / HashSet | [→](../../problems/128-longest-consecutive-sequence/problem.md) |
| 19 | Remove Nth Node From End of List | Med | not started | - | - |
| 143 | Reorder List | Med | not started | - | - |

---

## Phase 2 - New Thinking Layers (Problems 21-40)

### Batch 6 - Trees Entry

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 104 | Maximum Depth of Binary Tree | Easy | not started | - | - |
| 100 | Same Tree | Easy | not started | - | - |
| 226 | Invert Binary Tree | Easy | not started | - | - |
| 102 | Binary Tree Level Order Traversal | Med | not started | - | - |
| 105 | Construct Binary Tree from Preorder and Inorder Traversal | Med | not started | - | - |

### Batch 7 - BST

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 230 | Kth Smallest Element in a BST | Med | not started | - | - |
| 98 | Validate Binary Search Tree | Med | not started | - | - |
| 235 | Lowest Common Ancestor of a Binary Search Tree | Med | not started | - | - |
| 572 | Subtree of Another Tree | Easy | not started | - | - |

### Batch 8 - Graphs Entry

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 200 | Number of Islands | Med | not started | - | - |
| 133 | Clone Graph | Med | not started | - | - |
| 261 | Graph Valid Tree | Med | not started | - | - |
| 323 | Number of Connected Components in an Undirected Graph | Med | not started | - | - |

### Batch 9 - Backtracking + Advanced Graphs

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 207 | Course Schedule | Med | not started | - | - |
| 417 | Pacific Atlantic Water Flow | Med | not started | - | - |
| 39 | Combination Sum | Med | not started | - | - |
| 79 | Word Search | Med | not started | - | - |

### Batch 10 - Dynamic Programming Entry

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 70 | Climbing Stairs | Easy | not started | - | - |
| 198 | House Robber | Med | not started | - | - |
| 213 | House Robber II | Med | not started | - | - |
| 322 | Coin Change | Med | not started | - | - |

---

## Phase 3 - Interview Level (Problems 41-68)

### Batch 11 - DP Depth

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 300 | Longest Increasing Subsequence | Med | not started | - | - |
| 139 | Word Break | Med | not started | - | - |
| 91 | Decode Ways | Med | not started | - | - |
| 62 | Unique Paths | Med | not started | - | - |

### Batch 12 - DP + Greedy Mix

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 1143 | Longest Common Subsequence | Med | not started | - | - |
| 152 | Maximum Product Subarray | Med | not started | - | - |
| 55 | Jump Game | Med | not started | - | - |
| 252 | Meeting Rooms | Easy | not started | - | - |

### Batch 13 - Intervals

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 253 | Meeting Rooms II | Med | not started | - | - |
| 56 | Merge Intervals | Med | not started | - | - |
| 57 | Insert Interval | Med | not started | - | - |
| 435 | Non-overlapping Intervals | Med | not started | - | - |

### Batch 14 - Heaps + Design

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 347 | Top K Frequent Elements | Med | not started | - | - |
| 295 | Find Median from Data Stream | Hard | not started | - | - |
| 271 | Encode and Decode Strings | Med | not started | - | - |
| 297 | Serialize and Deserialize Binary Tree | Hard | not started | - | - |

### Batch 15 - Trie

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 208 | Implement Trie (Prefix Tree) | Med | not started | - | - |
| 211 | Design Add and Search Words Data Structure | Med | not started | - | - |
| 212 | Word Search II | Hard | not started | - | - |
| 269 | Alien Dictionary | Hard | not started | - | - |

### Batch 16 - Matrix + Bit Manipulation Entry

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 54 | Spiral Matrix | Med | not started | - | - |
| 73 | Set Matrix Zeroes | Med | not started | - | - |
| 48 | Rotate Image | Med | not started | - | - |
| 191 | Number of 1 Bits | Easy | not started | - | - |

### Batch 17 - Final Stretch

| LC # | Problem | Difficulty | Status | Pattern | Link |
|------|---------|-----------|--------|---------|------|
| 190 | Reverse Bits | Easy | not started | - | - |
| 338 | Counting Bits | Easy | not started | - | - |
| 371 | Sum of Two Integers | Med | not started | - | - |
| 124 | Binary Tree Maximum Path Sum | Hard | not started | - | - |
